...
//		EmployeeService service = new EmployeeArray();
		...
		while (true) {
			System.out.println("1.사원수(초기화) 2.사원정보입력 3.사원검색 4.사원리스트 5.사원급여검색 9.종료");
			System.out.print("선택 >>> ");
			
			// 숫자가 아닌 문자가 입력되었을 경우 예외처리하기(try-catch)
			int menu = -1;
			try {
				menu = Integer.parseInt(scn.nextLine()); // "1" -> 1 문자열을 숫자로 바꿔주는 기능
